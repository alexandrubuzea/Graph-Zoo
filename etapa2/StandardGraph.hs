...
-}
data StandardGraph a = StdGraph -- a = (S.Set a, S.Set (a, a))
    {
        graphNodes :: S.Set a, -- cele 2 functii membru in tipul nostru de date
        graphEdges :: S.Set (a, a) -- "getterii"
    } deriving Show -- just in case ca vrem sa afisam :)

{-
    Am instantiat clasa Haskell Eq asta pentru a face comparatia
    de egalitate intre cele 2 grafuri (functie de test -
    functia shouldBeTrue de mai jos)
-}
instance Eq a => Eq (StandardGraph a) where
    StdGraph n1 e1 == StdGraph n2 e2 = n1 == n2 && e1 == e2
    g1 /= g2 = not (g1 == g2)
{-
    *** TODO ***
...
removeNode node graph = StdGraph newNodes newEdges
    where
        newNodes = S.filter (\x -> x /= node) $ nodes graph
        newEdges = S.filter (\x -> fst x /= node && snd x /= node) $ edges graph
...
splitNode old news graph = StdGraph newNodes newEdges
    where
        newNodes = S.union (nodes $ removeNode old graph) (S.fromList news)
        newEdges = S.union addedEdges (edges $ removeNode old graph)
        addedEdges = S.fromList ([(x, y) | x <- S.toList $ inNeighbors old graph, y <- news] ++ [(x, y) | x <- news, y <- S.toList $ outNeighbors old graph]) 
...
mergeNodes :: Ord a
           ...
           -> StandardGraph a  -- graful existent
           -> StandardGraph a  -- graful obținut
mergeNodes prop node graph = StdGraph newNodes newEdges
    where
        newNodes = if (not . null) deletedNodes then S.union (S.filter (not . prop) $ nodes graph) (S.fromList [node]) else nodes graph
        deletedNodes = S.toList $ S.filter prop $ nodes graph
        myFunc = (\x -> if prop x == True then node else x)
        addedEdges = S.fromList ([(x, node) | nd <- deletedNodes, x <- S.toList $ S.map myFunc $ inNeighbors nd graph] ++
            [(node, x) | nd <- deletedNodes, x <- S.toList $ S.map myFunc $ outNeighbors nd graph])
        newEdges = S.union addedEdges $ S.filter (\(x, y) -> not (elem x deletedNodes) == True && not (elem y deletedNodes) == True) $ edges graph