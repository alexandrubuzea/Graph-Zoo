...
splitNode node targets = extend (\x -> if x == node then foldr Overlay Empty $ map Node targets else Node x)
...
mergeNodes :: (a -> Bool)       -- proprietatea îndeplinită de nodurile îmbinate
           ...
           -> AlgebraicGraph a  -- graful existent
           -> AlgebraicGraph a  -- graful obținut
mergeNodes prop node = fmap (\x -> if prop x == True then node else x)
...
filterGraph prop = extend (\x -> if prop x == True then Node x else Empty)
