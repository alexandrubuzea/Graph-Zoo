...
mapSingle :: (a -> a) -> [a] -> [[a]]
mapSingle f [] = []
mapSingle f (x:xs) = [(f x) : xs] ++ (map (x:) $ mapSingle f xs)
...
partitions [] = []
partitions [x] = [[[x]]]
partitions (x:xs) = concatMap (\part -> ([x] : part) : (mapSingle (x:) part)) $ partitions xs
...
isModule :: Ord a
         ...
         -> Graph a
         -> Bool
isModule set graph = if countOuter <= 1 && countInner <= 1 then True else False
    where
        outerNeigh = S.map ((\x -> S.filter (\y -> not (S.member y set)) x) . ((flip outNeighbors) graph)) $ set 
        countOuter = S.size outerNeigh
        innerNeigh = S.map ((\x -> S.filter (\y -> not (S.member y set)) x) . ((flip inNeighbors) graph)) $ set
        countInner = S.size innerNeigh
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = if results == (S.fromList [True]) then True else False
    where
        results = S.map (flip isModule $ graph) partition
...
maximalModularPartition :: Ord a
                        ...
                        -> Graph a
                        -> Partition a
maximalModularPartition partitions graph = result
    where
        candidates1 = filter (flip isModularPartition $ graph) partitions
        candidates2 = filter (\x -> length x /= 1) candidates1
        result = minimumBy (compare `on` S.size) candidates2
{-
