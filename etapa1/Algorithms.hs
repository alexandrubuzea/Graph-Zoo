...
-}
searchVisited :: Ord a => ([a] -> [a] -> [a]) -> a -> S.Set a -> [a] -> Graph a -> [a]
searchVisited f node visited [] graph = []
searchVisited f node visited struct graph = (if S.member node visited then [] else [node]) ++ searchVisited f newNode newVisited newStruct graph
    where
        newNode = head newStruct
        newVisited = S.insert node visited
        myFunc = (\x -> not $ S.member x visited)
        newStruct = f (tail struct) $ S.toList $ S.filter myFunc (StandardGraph.outNeighbors node graph)
...
search f node graph = searchVisited f node S.empty [node] graph
...
dfs = search (\x y -> y ++ x) -- mergea si cu flip (++) I suppose
...
countIntermediate from to graph = if elem to listBFS then Just (bfsLen, dfsLen) else Nothing
    where
        listBFS = bfs from graph
        listDFS = dfs from graph
        bfsLen = (length $ takeWhile (\x -> x /= to) listBFS) - 1
        dfsLen = (length $ takeWhile (\x -> x /= to) listDFS) - 1
